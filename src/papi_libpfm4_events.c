...
_papi_libpfm4_error( int pfm_error ) {
  ...
  default: 
    PAPIWARN("Unknown libpfm error code %d, returning PAPI_EINVAL");
    return PAPI_EINVAL;
  }
}
...
_papi_libpfm4_init(papi_vector_t *my_vector) {
   ...
   if (!libpfm4_users) {
	...
	strncpy(my_vector->cmp_info.disabled_reason,pfm_strerror(retval),PAPI_MAX_STR_LEN); 
	_papi_hwi_unlock( NAMELIB_LOCK );
	return PAPI_ESBSTR;
      }
   } else {
   ...
   if ((version=pfm_get_version( )) >= 0) {
     ...
     if ( PFM_MAJ_VERSION( version ) != PFM_MAJ_VERSION( LIBPFM_VERSION ) ) {
       PAPIWARN( "Version mismatch of libpfm: compiled %#x vs. installed %#x\n",
